...
# from getTestInfo import add_testInfo, testInfo

# INSERIR SHITTENS AQUI, MEU SAGAZ
...
class TestCamp:
    ...
    def __str__(self):
        return 'The test campaign {} occured in {} test bench as a {}\'s request to analyze the performance of the {}.'.format(self.name, self.bnch, self.clnt, self.spcm)
    ...
    def fullname(self):
        ...
        if not self.name == self.spcm:
            fullname += dlmt + self.spcm

        fullname += dlmt + self.bnch

        if self.TD_lst:
            if self.TD_lst[0].date == self.TD_lst[-1].date:
                fullname += dlmt + str(self.TD_lst[0].year)
            else:
                fullname += dlmt + str(self.TD_lst[0].year) + '-' + str(self.TD_lst[-1].year)
    ...
    def allocate_data_2_fldr_strt(self, root_folder):
        ...
        TC_folder = osp.join(root_folder, self.fullname())
        if not osp.exists(TC_folder):
            os.makedirs(TC_folder)

        for td in self.TD_lst:
            TD_folder = osp.join(TC_folder, td.fullname())
            if not osp.exists(TD_folder):
                os.makedirs(TD_folder)
            for run in td.Run_lst:
                Run_folder = osp.join(TD_folder, run.fullname())
    ...
    def find_ts(self, file):
        ...
        if daq.name == daqName1 or daq.name == daqName2:
            file_name = file_name.replace(daq.pref, '').replace(daq.suff, '').split(dlmt)
    ...
    def extract_ts_filename(file_name, prefix, suffix, time_stamp_format):
        ...
        return datetime.strptime(file_name.replace(prefix + dlmt, '').replace(dlmt + suffix, ''), time_stamp_format)
...
class TestDay:
    ...
    def fullname(self):
        return 'TestDay' + str(self.rel_counter).zfill(2) + dlmt + self.name
...
class Run:
    ...
    def fullname(self):
        return 'Run' + str(self.rel_counter).zfill(2) + dlmt + 'absRun' + str(self.abs_counter).zfill(3) + dlmt + self.name
...
# Globals
template_repo_path = 'XXX/template_repo'
